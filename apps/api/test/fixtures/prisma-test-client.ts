...
export class InMemoryPrisma {
  ...
  // $transaction supports both the callback form and the array form. Atomicity
  // is not modeled (single-threaded JS, no concurrent writers in tests).
  async $transaction<T>(arg: ((tx: InMemoryPrisma) => Promise<T>) | Promise<unknown>[]): Promise<T | unknown[]> {
