...
1. **Throughput is both covered and unresolved.** Success Criteria says:
   `"Observability-vocabulary check: in /console the reviewer can point at concrete surfaces for Latency, Throughput, and Errors ... per-hour aggregation on the Cost sparkline cover all three."`
   Open Questions says:
