...
- > `pnpm --filter @ollive/api install`

  This is likely wrong. `pnpm install` is normally workspace-root only; filtered install is not usually the command. Use `pnpm install` then `pnpm --filter @ollive/api typecheck`.
