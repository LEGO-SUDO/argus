...
> Task 1B: “All Task 1A tests pass; `@ollive/contracts` exports every name…”
...
The biggest test gap is test infrastructure itself. The LLD asks for many `apps/web/__tests__` files, but `@ollive/web` has no `test` script, Jest config, jsdom setup, or Testing Library dependencies. Add an early RED/GREEN or scaffold task for test setup.
