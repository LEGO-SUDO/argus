...
3. D3 SSE: simpler than `LISTEN/NOTIFY` is SSE with short server-side polling on `MAX(updated_at)` per user. At take-home scale, this avoids DB subscription lifecycle bugs.
