...
**Rationale:** Phase A already stores costs as integer micro-USD with a `(user_id, created_at)` index; at take-home volumes a per-user SUM is sub-50ms. A rollup adds a second source of truth and a staleness window that conflicts with the live promise.
