...
- **Primary — Ollive hiring panel.** ~3 minutes in `/console`, deciding whether the submission clears the senior-IC bar.
- **Secondary — the candidate, post-hire.** The template for Ollive's first volume customer.
...
- JD-vocabulary check: the reviewer can point at concrete surfaces in `/console` for *Latency* (Traces row), *Throughput* (Traces strip), and *Errors* (Traces strip and status column).
